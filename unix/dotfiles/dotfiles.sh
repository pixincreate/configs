...
linux() {
  sudo apt-get update && sudo apt-get install croc git-delta micro neovim starship walk zoxide
}

mac() {
  brew install croc git-delta micro neovim starship walk zoxide
}
...
cat > ~/.gitconfig <<'EOF'
  ...
  [user]
  email = 69745008+pixincreate@users.noreply.github.com
  name = PiX
